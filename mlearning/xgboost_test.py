...
for i in range(1):  # 10 iterations
    ...
    for i in range(40):
        ...
        D_train = xgb.DMatrix(X_train_np, label=y_train_np)

        # Fit
        gb = xgb.train(params, D_train, steps)
        ...
        D_test = xgb.DMatrix(X_test_np, label=y_test.to_numpy())

        pred = gb.predict(D_test)
